-- 20260708130000_match_course_weight_tuning.sql
-- Reproduz calculate_match(uuid) da 20260708100000 com 2 ajustes de calibração (produto):
--   1. course_score: aproximado 85 -> 75; grupo de área 70 -> 50 (o match exato de curso
--      passa a se diferenciar mais dos aproximados/vizinhos de área).
--   2. Pilar de preferência: turno 1/3 -> 1/4, prog/univ 1/3 -> 1/4, curso 1/3 -> 1/2
--      (o curso de interesse dobra de importância dentro das preferências).
    ...
    scored_preferences AS (
        SELECT sa.*,
            ...
            CASE
                ...
                WHEN v_course_interests IS NOT NULL AND cardinality(v_course_interests) > 0
                     AND EXISTS (SELECT 1 FROM unnest(v_course_interests) ci
                                 WHERE sa.course_name ILIKE '%'||ci||'%') THEN 75.0
                WHEN v_course_interests IS NOT NULL AND cardinality(v_course_interests) > 0
                     AND v_course_group_courses IS NOT NULL
                     AND UPPER(sa.course_name) = ANY(v_course_group_courses) THEN 50.0
                   ...
                   ELSE LEAST(100.0, GREATEST(0.0,
                       (
                         ...
                         + COALESCE((v_weights->>'preference_weight')::NUMERIC,0.30) * (
                             sl.shift_score*0.25 + sl.inst_program_score*0.25 + sl.course_score*0.50)
