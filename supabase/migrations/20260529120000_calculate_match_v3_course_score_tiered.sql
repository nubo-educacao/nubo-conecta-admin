-- Migration: calculate_match V3 — tiered course_score
-- Sprint 15.0 — Plan C fix #3
--
-- Problem: all courses matching via ILIKE '%medicina%' receive course_score=100.
-- "Medicina", "Medicina Veterinária", "Biomedicina" are treated identically.
--
-- Fix: 3-tier scoring by match specificity:
--   exact match  (LOWER(course_name) = LOWER(interest))     → 100
--   starts with  (course_name ILIKE interest||'%')           →  85
--   contains     (course_name ILIKE '%'||interest||'%')      →  70
--   no match                                                  →  10
