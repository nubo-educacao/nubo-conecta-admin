-- 20260609145000_rawemec_no_truncate.sql
-- rawemec is a permanent reference database (all IES in Brazil).
-- Unlike rawsisu/rawprouni (per-cycle temporary data), rawemec should never be truncated after ETL.
-- Remove the TRUNCATE from etl_import_emec so the data persists across cycles.
...
BEGIN
  ...
  IF NOT v_has_more THEN
    ...
    IF v_errors IS NULL THEN
      ...
      -- NOTE: rawemec is a permanent reference table and is intentionally NOT truncated here.
      -- It holds all IES from Brazil and must persist so new institutions can be enriched at any time.
    ELSE
