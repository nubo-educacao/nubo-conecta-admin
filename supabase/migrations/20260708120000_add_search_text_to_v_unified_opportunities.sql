-- 20260708120000_add_search_text_to_v_unified_opportunities.sql
--
-- Adiciona coluna search_text à v_unified_opportunities (Materialized View).
-- search_text = f_unaccent(title || ' ' || provider_name || ' ' || location)
-- pré-computado na matview para que busca accent-insensitive seja feita
-- diretamente sobre a coluna (sem recalcular f_unaccent() por linha na query).
--
-- Estratégia (sem DROP CASCADE):
--   1. Dropa apenas as dependências diretas da matview (funções que usam o shape):
--      get_unified_opportunities_by_distance, search_opportunities
--   2. DROP + CREATE da matview com search_text adicionado
--   3. Recria os índices (unique + institution + GIN em search_text)
--   4. Recria as funções consumidoras atualizadas
--   5. REFRESH da matview p/ popular search_text nos dados existentes
--
-- Baseado na estrutura canônica de 20260706130000_restore_unified_views.sql
...
SELECT sisu_branch.* FROM (
  SELECT DISTINCT ON (c.id)
    ...
    -- NOVA COLUNA: search_text pré-computado para busca accent-insensitive eficiente
    public.f_unaccent(
...
SELECT prouni_branch.* FROM (
  SELECT DISTINCT ON (c.id)
    ...
    -- NOVA COLUNA: search_text pré-computado
    public.f_unaccent(
      COALESCE(c.course_name, '') || ' ' ||
      COALESCE(i.name, '') || ' ' ||
      COALESCE(cp.city, '') || ' ' ||
...
SELECT
  ...
  -- NOVA COLUNA: search_text pré-computado (parceiros buscam por nome e instituição)
  public.f_unaccent(
    COALESCE(po.name, '') || ' ' ||
    COALESCE(i.name, '')  || ' ' ||
    COALESCE(sis.acronym, '')
...
-- 2. Índices
-- ====================================================================================
-- Índice único para REFRESH CONCURRENTLY (ADR-0019)
...
-- ====================================================================================
-- 4. get_unified_opportunities_by_distance — atualizado com search_text no RETURNS TABLE
-- ====================================================================================
