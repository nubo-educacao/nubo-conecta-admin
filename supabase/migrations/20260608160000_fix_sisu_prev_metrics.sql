-- Migration: Fix qt_inscricao_prev and vagas_ociosas in v_unified_opportunities (SISU branch)
-- 20260608160000_fix_sisu_prev_metrics.sql
--
-- Bugs fixed:
-- 1. qt_inscricao_prev was reading sv_curr.qt_inscricao_prev (column never populated by ETL → always NULL)
--    Fix: add lateral join o_prev to read raw_data->>'QT_INSCRICAO' from previous cycle year
-- 2. vagas_ociosas_current was hardcoded as NULL::integer
--    Fix: calculate as sv_curr.qt_vagas_ofertadas::integer - (o.raw_data->>'QT_INSCRICAO')::integer
-- 3. vagas_ociosas_prev was reading sv_curr.vagas_ociosas_prev (column never populated → always NULL)
...
-- 2. Recreate v_unified_opportunities with corrected SISU prev metrics
...
FROM (
    SELECT DISTINCT ON (c.id)
        ...
        -- Vagas ofertadas (current / prev)
        sv_curr.qt_vagas_ofertadas AS qt_vagas_ofertadas_current,
        sv_prev.qt_vagas_ofertadas AS qt_vagas_ofertadas_prev,

        -- Inscrições current: from current year opportunity raw_data
        -- Note: QT_INSCRICAO uses dot as thousands separator (e.g. "1.195" = 1195), remove before cast
        replace(o.raw_data->>'QT_INSCRICAO', '.', '')::text AS qt_inscricao_current,

        -- Inscrições prev: from previous cycle year opportunity raw_data (fixed — was sv_curr.qt_inscricao_prev, always NULL)
        o_prev.qt_inscricao AS qt_inscricao_prev,

        -- Vagas ociosas current: vagas_ofertadas - inscricoes (fixed — was hardcoded NULL)
        -- Uses replace() to handle dot-as-thousands-separator in QT_INSCRICAO
        CASE
            WHEN sv_curr.qt_vagas_ofertadas IS NOT NULL
             AND (o.raw_data->>'QT_INSCRICAO') IS NOT NULL
            THEN (replace(sv_curr.qt_vagas_ofertadas, '.', '')::integer
                  - replace(o.raw_data->>'QT_INSCRICAO', '.', '')::integer)
            ELSE NULL::integer
        END AS vagas_ociosas_current,

        -- Vagas ociosas prev: vagas_ofertadas_prev - inscricoes_prev (fixed — was sv_curr.vagas_ociosas_prev, always NULL)
        CASE
            WHEN sv_prev.qt_vagas_ofertadas IS NOT NULL
             AND o_prev.qt_inscricao IS NOT NULL
            THEN (replace(sv_prev.qt_vagas_ofertadas, '.', '')::integer
                  - replace(o_prev.qt_inscricao, '.', '')::integer)
            ELSE NULL::integer
        END AS vagas_ociosas_prev
    ...
    FROM public.opportunities o
        ...
        JOIN public.courses c
            ON c.id = o.course_id
        JOIN public.campus cp
            ON cp.id = c.campus_id
        JOIN public.institutions i
            ON i.id = cp.institution_id

        -- Cutoff scores current cycle
        LEFT JOIN LATERAL (
            SELECT min(opp.cutoff_score) AS min_cutoff,
                   max(opp.cutoff_score) AS max_cutoff
            FROM public.opportunities opp
            WHERE opp.opportunity_type = 'sisu'::text
              AND opp.course_id = o.course_id
              AND opp.year = p.cycle_year
        ) s_curr ON true

        -- Cutoff scores previous cycle
        LEFT JOIN LATERAL (
            SELECT min(opp.cutoff_score) AS min_cutoff,
                   max(opp.cutoff_score) AS max_cutoff
            FROM public.opportunities opp
        ...
        -- Previous cycle opportunity: to read qt_inscricao from raw_data (FIXED)
        -- Note: apply replace() to strip dot thousands-separator before returning
        LEFT JOIN LATERAL (
            SELECT replace(opp.raw_data->>'QT_INSCRICAO', '.', '') AS qt_inscricao
            FROM public.opportunities opp
            WHERE opp.course_id = o.course_id
              AND opp.opportunity_type = 'sisu'
              AND opp.year = (p.cycle_year - 1)
    ...
    WHERE o.opportunity_type = 'sisu'::text
...
FROM (
    ...
    FROM public.opportunities o
        ...
        LEFT JOIN LATERAL (
            SELECT min(opp.cutoff_score) AS min_cutoff,
                   max(opp.cutoff_score) AS max_cutoff
            FROM public.opportunities opp
            WHERE opp.opportunity_type = 'prouni'::text
              AND opp.course_id = o.course_id
              AND opp.year = p.cycle_year
        ) s_curr ON true
        ...
        -- ProUni vacancies current cycle: vagas and ociosas
        LEFT JOIN LATERAL (
            SELECT
        ...
        -- ProUni vacancies previous cycle
        LEFT JOIN LATERAL (
            SELECT
                sum(pv.bolsas_ampla_ofertada + pv.bolsas_cota_ofertada)::text AS qt_vagas_ofertadas,
                sum((pv.bolsas_ampla_ofertada + pv.bolsas_cota_ofertada) - (pv.bolsas_ampla_ocupada + pv.bolsas_cota_ocupada))::integer AS vagas_ociosas
    ...
    WHERE o.opportunity_type = 'prouni'::text
      AND o.year = p.cycle_year
      AND o.semester = p.cycle_semester
    ORDER BY c.id, o.created_at
) prouni_branch
...
-- 3. Recreate v_unified_institutions (depends on v_unified_opportunities)
...
-- 4. Recreate get_unified_opportunities_by_distance
