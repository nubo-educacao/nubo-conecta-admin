-- 20260828110000_prouni_etl_concurrency_type.sql
-- Nova versão do ETL ProUni com grain por concurrency_type.
--
-- MUDANÇAS vs versão anterior (20260708110000):
--   ? Grain: (course_id, shift, scholarship_type, concurrency_type) ? era (... concurrency_type IS NULL)
--   ? concurrency_type = normalização de TP_MODALIDADE:
--       'AMPLA' ? 'AMPLA'
--       'PPI'   ? 'COTA_PPI'
--       'PCD'   ? 'COTA_PCD'
--   ? cutoff_score = NU_NOTA_CORTE por modalidade (era NULL forçado)
--   ? Vacâncias: qt_ofertada = "Bolsas Ofertadas", qt_ocupada = "Bolsas Ocupadas"
--     (sem bolsas_ampla_ofertada/bolsas_cota_ofertada)
--   ? rawprouni PRECISA ter colunas: TP_MODALIDADE, NU_NOTA_CORTE, "Bolsas Ofertadas", "Bolsas Ocupadas"
--     (o merge_prouni.py já produz isso no prouni_2026_1_merged.csv)
...
BEGIN
  ...
  BEGIN
    ...
    WITH mapped_raw AS (
      SELECT
        ...
        -- Normaliza TP_MODALIDADE ? concurrency_type
        ...
        -- Tags de concorrência via regra centralizada
        ...
        -- Nota de corte por modalidade (agora disponível no CSV do MEC)
    ...
    inserted AS (
      ...
      ON CONFLICT ON CONSTRAINT uq_opportunities_prouni_concurrency DO NOTHING
      RETURNING id
    )
    SELECT count(*) INTO v_total_processed_in_log FROM inserted;

    -- 5. ProUni Vacancies ? 1 por oportunidade (concurrency_type já está na opp)
    --    qt_ofertada = "Bolsas Ofertadas" (total da modalidade neste curso)
    --    qt_ocupada  = "Bolsas Ocupadas"
    WITH vacancies_agg AS (
      SELECT
        o.id AS opportunity_id,
        -- Cada linha do CSV já representa UMA modalidade ? MAX para idempotência
  ...
  IF NOT v_has_more THEN
    -- Scholarship tags (mantém lógica existente)
...
-- ====================================================================================
-- Atualizar etl_clone_prouni_cycle e etl_rollback_log para remover colunas antigas
-- ====================================================================================
...
BEGIN
  ...
  BEGIN
    ...
    -- 2. Clone opportunities_prouni_vacancies by matching source opp ? new opp via natural keys
    WITH source_opps AS (
