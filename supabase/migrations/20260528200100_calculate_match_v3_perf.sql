-- Migration: calculate_match V3 — Performance fix
-- Sprint 15.0 — Plan C hotfix
--
-- Problem: 148k opportunities in prod caused timeout due to:
--   1. Path C: ORDER BY RANDOM() requires full-table sort O(N log N)
--   2. All 3 UNION ALL paths evaluated by PostgreSQL planner simultaneously
--
-- Fix:
--   1. Replace ORDER BY RANDOM() with hashtext(id || profile_id) — O(N), no sort, deterministic per user
--   2. Reduce LIMIT 3000→1500 and LIMIT 2000→1000 to halve CTE chain payload
...
BEGIN
    -- 1. Load user preferences (including quota_types)
    SELECT
    ...
    -- Fallback: treineiro scores (penalised 0.85x in academic_score)
    ...
    -- ── MEC funnel ────────────────────────────────────────────────────────────
    -- Note: all 3 paths use UNION ALL; PostgreSQL evaluates all branches.
    -- Path C previously used ORDER BY RANDOM() which required a full-table sort
    -- on 148k rows — replaced with hashtext for O(N) pseudo-random per user.
    mec_funnel AS (
        ...
        -- hashtext is O(N) vs ORDER BY RANDOM() which is O(N log N) on 148k rows
        SELECT * FROM (
            SELECT
                o.id AS opp_id,
                'mec_' || o.id::text AS unified_id,
            ...
            ORDER BY hashtext(o.id::text || p_profile_id::text)
            LIMIT 1000
        ) path_c
    ),

    -- ── Partners: approved/incoming/opened only (no 'closed') ────────────────
