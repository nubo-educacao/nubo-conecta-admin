-- Migration: calculate_match V3 — Fix unified_id + dynamic cycle from programs
-- Sprint 15.0 — Plan C bugfix
--
-- Bugs fixed:
--   1. unified_id was 'mec_' || o.id (opportunity_id) but v_unified_opportunities uses
--      'mec_' || c.id (course_id via DISTINCT ON c.id). JOIN never matched → 0 MEC results.
--   2. Hardcoded years (sisu=2026, prouni=2025) replaced with dynamic lookup from
--      programs WHERE status = 'opened', per type.
...
DECLARE
    ...
    -- Dynamic cycle from programs table
    v_sisu_year      INT;
    v_sisu_semester  TEXT;
    v_prouni_year    INT;
    v_prouni_semester TEXT;
BEGIN
    -- 0. Load active cycles from programs (status = 'opened')
    ...
    -- Fallback to known cycles if programs table is empty
    IF v_sisu_year IS NULL THEN
        v_sisu_year := 2025; v_sisu_semester := '1';
    END IF;
    IF v_prouni_year IS NULL THEN
        v_prouni_year := 2025; v_prouni_semester := '1';
    END IF;
    ...
    -- 2. Best ENEM scores — prefer official (is_treineiro = false), fallback to treineiro
    SELECT
        ues.nota_linguagens, ues.nota_ciencias_humanas,
        ues.nota_ciencias_natureza, ues.nota_matematica, ues.nota_redacao
    INTO
    ...
    -- ── MEC funnel ────────────────────────────────────────────────────────────
    -- IMPORTANT: unified_id = 'mec_' || c.id (course_id), matching v_unified_opportunities
    -- which uses DISTINCT ON (c.id). Do NOT use o.id here.
    mec_funnel AS (

        SELECT * FROM (
            SELECT
                o.id AS opp_id,
                'mec_' || c.id::text AS unified_id,  -- course_id, not opportunity_id
                o.course_id,
                c.course_name,
                o.opportunity_type,
                o.scholarship_type,
    ...
    all_opportunities AS (
        ...
        SELECT
            ...
            NULL::jsonb   AS concurrency_tags,
    ...
    -- ── Pilar 1: Performance & Elegibilidade ─────────────────────────────────
    scored_performance AS (
        SELECT
            ...
            END AS meets_income,
            CASE
                WHEN ao.concurrency_tags IS NULL
                     OR jsonb_array_length(ao.concurrency_tags) = 0 THEN false
        ...
        SELECT
            ...
            CASE
                ...
                WHEN sp.weighted_enem_score IS NOT NULL THEN
                    LEAST(100.0,
                        (sp.weighted_enem_score / 700.0) * 100.0
                        * CASE WHEN v_is_treineiro_score THEN 0.85 ELSE 1.0 END
                    )
                ELSE 50.0
    ...
    boosted AS (
        SELECT
            ...
            END
            + CASE
                WHEN c.cota_eligible AND c.meets_income THEN
                    COALESCE((v_weights->>'cota_bonus')::NUMERIC, 10.0)
                ELSE 0.0
            END AS final_score,
            jsonb_build_object(
                'meets_income',               c.meets_income,
                'cota_eligible',              c.cota_eligible,
                ...
                'cycle',                      jsonb_build_object(
                    'sisu_year', v_sisu_year, 'sisu_semester', v_sisu_semester,
