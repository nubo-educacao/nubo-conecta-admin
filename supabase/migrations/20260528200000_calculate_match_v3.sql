-- Migration: calculate_match V3
-- Sprint 15.0 — Plan C: Match Engine Qualification
--
-- Changes vs V2:
--   1. ENEM: prefer is_treineiro = false (official); fallback to is_treineiro = true with 0.85x penalty
--   2. Load user quota_types for OR-of-ANDs cota eligibility
--   3. OR-of-ANDs: cota_eligible computed from concurrency_tags structure → +10 cota_bonus
--   4. concurrency_tags added to mec_funnel paths A/B/C
--   5. Partners: 'closed' removed from status filter
...
BEGIN
    ...
    scored_performance AS (
        SELECT
            ao.*,
            -- Income eligibility (hard filter for ProUni/SiSU income cotas)
            CASE
                ...
                ELSE true
            END AS meets_income,
            -- OR-of-ANDs cota eligibility: user qualifies if their quota_types satisfy ANY
            -- inner tag set (all tags in that set must be in v_quota_types)
            CASE
            ...
            CASE
                WHEN sp.weighted_enem_score IS NOT NULL AND sp.cutoff_score IS NOT NULL AND sp.cutoff_score > 0 THEN
                    GREATEST(0.0, LEAST(100.0,
                        ...
                        -- Treineiro penalty: score carries less weight in ranking
    ...
    boosted AS (
        SELECT
            ...
            + CASE
                ...
                ELSE 0.0
            END
            -- Cota bonus: user qualifies for reserved slots → higher admission probability
            + CASE
