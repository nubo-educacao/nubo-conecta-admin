-- 20260706121000_match_prouni_agnostic.sql
-- Plan v2 (61b505ce) — Card 3: Match Engine agnóstico para ProUni.
-- Reproduz calculate_match(uuid) verbatim, com 3 edições cirúrgicas:
--   3.1 Funil por relevância — Path A ordena por (match de curso, proximidade); Path B por proximidade.
--       (antes: ORDER BY o.cutoff_score DESC NULLS LAST, que suprimia ProUni com corte NULL.)
--   3.2 Redistribuição de peso — na CTE composite, o base_score é a média ponderada dos pilares
--       COM DADO. Sem corte (ProUni), performance_weight=0 e pref/loc são renormalizados (÷ soma
--       dos pesos usados), dando teto 100 e paridade com o SiSU.
--   3.3 Gate de elegibilidade ENEM ≥ 450 para ProUni (regra do programa).
