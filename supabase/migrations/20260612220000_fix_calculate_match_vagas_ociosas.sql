-- Migration: Fix calculate_match — vagas_ociosas_2025 → has_vagas_ociosas boolean
-- Problema: a coluna vagas_ociosas_2025 foi removida de opportunities_sisu_vacancies
-- na migração year-agnostic. A RPC referenciava essa coluna em 5 lugares e falhava
-- em runtime. Substituído por boolean calculado inline via EXISTS, espelhando a
-- lógica já presente na matview v_unified_opportunities.
...
COMMENT ON FUNCTION public.calculate_match IS 'V4: Multi-factorial match engine. Iterates per opportunity with SISU weights, income eligibility, preferences, Haversine location, boosts. Uses asymmetric cutoff decay and returns best opportunity metadata. Aggregates MAX per course. Fix: vagas_ociosas calculado via EXISTS boolean (year-agnostic).';
