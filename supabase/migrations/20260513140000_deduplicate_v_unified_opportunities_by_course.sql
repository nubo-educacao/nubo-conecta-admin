-- Migration: Deduplicate v_unified_opportunities by course (c.id)
-- Problem: each opportunity row = one concurrency type (cota), causing N cards per course.
-- Fix: DISTINCT ON (c.id, o.opportunity_type) wrapped in subquery → 1 card per course per type.
--      unified_id changes from mec_<opportunity.id> to mec_<course.id>
-- Sprint 8.0
...
CREATE MATERIALIZED VIEW v_unified_opportunities AS
  ...
  -- Branch MEC: 1 row per course per opportunity_type, wrapped in subquery to allow UNION ALL
  SELECT * FROM (
    SELECT DISTINCT ON (c.id, o.opportunity_type)
    ...
    LEFT JOIN (
      ...
      FROM opportunities GROUP BY course_id
    ) s ON s.course_id = o.course_id
    LEFT JOIN LATERAL (
      SELECT d.start_date, d.end_date
      FROM important_dates d
      WHERE d.type = o.opportunity_type AND d.controls_opportunity_dates = true
    ...
    WHERE o.semester = '1'
      AND ((o.opportunity_type = 'sisu' AND o.year = 2026)
        OR (o.opportunity_type = 'prouni' AND o.year = 2025))
    ORDER BY c.id, o.opportunity_type, o.created_at
  ) mec_courses

  UNION ALL

  -- Branch Parceiro: unchanged (each partner_opportunity is already unique)
  SELECT
