-- 20260707130000_match_pref_city_geo_and_ead.sql
-- Reproduz calculate_match(uuid) da migration 20260706121000, com 2 edições:
--   A. Cidade de preferência como âncora geográfica — location_preference (+state_preference)
--      é resolvida em lat/long via public.cities e SOBRESCREVE as coordenadas do device.
--      Assim o funil geográfico (Path B) e o distance_score passam a pontuar oportunidades
--      PRÓXIMAS à cidade preferida (não só as daquela cidade, que só ganhavam +30 no
--      regional_bonus por igualdade de nome).
--   B. Preferência de turno EAD casa com os rótulos reais do MEC — 'EAD' nos preferred_shifts
--      é expandida para incluir 'Curso a distância' e 'EaD'. Antes, quem marcava EAD dava
--      match ZERO com ProUni EAD (shift='Curso a distância'): a opp era excluída do funil
--      Path A (filtro o.shift = ANY(...)) e recebia shift_score=0. Agora entra no funil e
--      recebe shift_score=100 — a pontuação extra para ProUni EAD pedida pelo produto.
