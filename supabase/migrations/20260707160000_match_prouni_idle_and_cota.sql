-- 20260707160000_match_prouni_idle_and_cota.sql
-- Reproduz calculate_match(uuid) da 20260707130000 (incluindo o patch bidirecional de EAD
-- da 20260707150000), fechando os 2 gaps ProUni registrados no Plan v2 (61b505ce):
--   A. idle_vacancy_boost ProUni-aware: has_vagas_ociosas no funil (3 paths) passa a
--      considerar também opportunities_prouni_vacancies (ofertadas > ocupadas). Antes só
--      consultava opportunities_sisu_vacancies → ProUni nunca ganhava o +5, divergindo da
--      tag "Vagas Ociosas" do card (que vem da matview, ProUni-aware).
--   B. cota_bonus ProUni-aware: cota_eligible passa a ser true para ProUni quando o
--      usuário declarou algum quota_type E a opportunity tem bolsas_cota_ofertada > 0
--      (o split Ampla/Cotas do ProUni vive nas vacancies, não em concurrency_tags).
...
BEGIN
    ...
    scored_performance AS (
        SELECT
            ...
            END AS meets_income,
            CASE
                -- ProUni: modalidade de cota vive em opportunities_prouni_vacancies (não em
                -- concurrency_tags). Elegível se o usuário declarou algum quota_type e a
                -- opportunity oferta vagas de cota.
                WHEN ao.opportunity_type = 'prouni' THEN
                    (cardinality(COALESCE(v_quota_types, '{}'::text[])) > 0
                     AND EXISTS (
