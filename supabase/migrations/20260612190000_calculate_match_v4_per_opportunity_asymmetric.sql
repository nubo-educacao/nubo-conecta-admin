-- Migration: calculate_match V4 — Per-Opportunity Scoring + Asymmetric Cutoff + Status != 'inactive'
-- Description: Updates the calculate_match RPC to include score decay configs, asymmetric cutoff scoring,
--              status != 'inactive' for program selection, and exports the best opportunity metadata.

-- 1. Insert new configuration values if not exists
INSERT INTO public.match_config (weight_key, weight_value, description, category) VALUES
  ('score_decay_above_cutoff', 0.3, 'Penalidade por ponto acima da nota de corte', 'score_decay'),
  ('score_decay_below_cutoff', 0.9, 'Penalidade por ponto abaixo da nota de corte', 'score_decay')
ON CONFLICT (weight_key) DO UPDATE SET
  weight_value = EXCLUDED.weight_value,
  description = EXCLUDED.description,
  category = EXCLUDED.category;

-- 2. Drop and recreate the calculate_match RPC
...
BEGIN
    -- 0. Active cycles from programs (status <> 'inactive')
    ...
    -- 2. ENEM — official first, treineiro fallback
    ...
    -- 6. Populate MEC funnel via IF/ELSIF — only ONE path executes
    ...
    IF v_has_funnel_filters THEN
        -- Path A: funnel filters active — indexed by cutoff_score
        ...
        SELECT o.id, 'mec_'||c.id::text, o.course_id, c.course_name,
               ...
               sv.peso_matematica, sv.peso_redacao, sv.vagas_ociosas_2025, NULL::jsonb
        ...
        WHERE (
          ...
          AND (v_course_interests IS NULL OR cardinality(v_course_interests) = 0
               OR EXISTS (SELECT 1 FROM unnest(v_course_interests) ci
    ...
    ELSIF v_lat IS NOT NULL AND v_lon IS NOT NULL THEN
        -- Path B: no filters, has geolocation → haversine ≤ 500 km
        INSERT INTO _mec_funnel
        SELECT o.id, 'mec_'||c.id::text, o.course_id, c.course_name,
               o.opportunity_type, o.scholarship_type, o.concurrency_type, o.concurrency_tags,
               o.cutoff_score, o.shift, false,
    ...
    ELSE
        -- Path C: no filters, no location → hash-based pseudo-random per user
        INSERT INTO _mec_funnel
        SELECT o.id, 'mec_'||c.id::text, o.course_id, c.course_name,
               o.opportunity_type, o.scholarship_type, o.concurrency_type, o.concurrency_tags,
               o.cutoff_score, o.shift, false,
    ...
    all_opportunities AS (
        ...
        SELECT po.id, 'partner_'||po.id::text, NULL::uuid, po.name,
               ...
               po.institution_id, NULL, NULL, NULL, NULL, NULL, NULL::integer,
    ...
    scored_preferences AS (
        SELECT sa.*,
            ...
            -- Tiered course name matching: exact > starts-with > contains > no match
            CASE
                WHEN v_course_interests IS NULL OR cardinality(v_course_interests)=0 THEN 50.0
                WHEN EXISTS (SELECT 1 FROM unnest(v_course_interests) ci
                             WHERE LOWER(sa.course_name) = LOWER(ci)) THEN 100.0
                WHEN EXISTS (SELECT 1 FROM unnest(v_course_interests) ci
                             WHERE sa.course_name ILIKE ci||' %'
                                OR sa.course_name ILIKE ci||' -%'
                                OR LOWER(sa.course_name) = LOWER(ci)) THEN 85.0
                WHEN EXISTS (SELECT 1 FROM unnest(v_course_interests) ci
                             WHERE sa.course_name ILIKE '%'||ci||'%') THEN 70.0
                ELSE 10.0
            END AS course_score
    ...
    composite AS (
        SELECT sl.opp_id, sl.concurrency_type, sl.concurrency_tags,
               ...
               sl.vagas_ociosas_2025, sl.opportunity_type,
    ...
    boosted AS (
        SELECT c.*,
            ...
            + CASE WHEN c.vagas_ociosas_2025 IS NOT NULL AND c.vagas_ociosas_2025>0 AND c.meets_income THEN
...
COMMENT ON FUNCTION public.calculate_match IS 'V4: Multi-factorial match engine. Iterates per opportunity with SISU weights, income eligibility, preferences, Haversine location, boosts. Uses asymmetric cutoff decay and returns best opportunity metadata. Aggregates MAX per course.';
