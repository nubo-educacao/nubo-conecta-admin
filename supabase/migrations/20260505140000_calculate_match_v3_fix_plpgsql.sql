-- =============================================================================
-- Migration: calculate_match V3 — Fix PL/pgSQL bare SELECT + single INSERT
--
-- Bug: "query has no destination for result data" em linha 104
-- Causa: SELECT sem INTO dentro de PL/pgSQL no encerramento do CTE duplo.
-- Fix: substituir dois INSERTs via CTE por um único INSERT com UNION ALL interno
--      (TOP 100 MEC + ALL partners em um só statement — sem SELECT órfão).
-- =============================================================================
...
COMMENT ON FUNCTION public.calculate_match IS
    'V3 Funnel+Cap: single INSERT via UNION ALL (TOP 100 MEC + ALL partners). '
    'MEC capped at 3000 rows per path (ORDER BY cutoff_score DESC). '
    'Fix: replaced dual data-modifying CTEs (invalid in PL/pgSQL) with single INSERT+WITH.';
