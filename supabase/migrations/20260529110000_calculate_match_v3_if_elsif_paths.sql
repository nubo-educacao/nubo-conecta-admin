-- Migration: calculate_match V3 — IF/ELSIF path selection via temp table
-- Sprint 15.0 — Plan C perf fix #2
--
-- Root cause of timeout: UNION ALL in CTE causes PostgreSQL to execute ALL 3 paths
-- simultaneously because v_has_funnel_filters is a PL/pgSQL variable (not a SQL constant).
-- Even paths returning 0 rows still do full Seq Scan + JOIN on 148k opportunities.
--
-- Fix: use IF/ELSIF to populate a temp table with only the relevant path.
-- PostgreSQL only executes ONE branch — reduces work by ~3x.
...
BEGIN
    -- 0. Active cycles from programs (status = 'opened')
    SELECT cycle_year, cycle_semester INTO v_sisu_year, v_sisu_semester
    FROM public.programs WHERE LOWER(type) = 'sisu' AND status = 'opened'
    ...
    FROM public.programs WHERE LOWER(type) = 'prouni' AND status = 'opened'
    ORDER BY cycle_year DESC, cycle_semester DESC LIMIT 1;
    ...
    IF v_weights IS NULL THEN
        v_weights := '{"performance_weight":0.40,"preference_weight":0.30,"location_weight":0.20,
                       "partner_boost":1.15,"partner_boost_cap":20,"idle_vacancy_boost":5,"cota_bonus":10}'::jsonb;
    END IF;
    ...
    scored_preferences AS (
        SELECT sa.*,
            ...
            CASE WHEN v_course_interests IS NULL OR cardinality(v_course_interests)=0 THEN 50.0
                 WHEN EXISTS (SELECT 1 FROM unnest(v_course_interests) ci
                              WHERE sa.course_name ILIKE '%'||ci||'%') THEN 100.0
                 ELSE 10.0 END AS course_score
    ...
    composite AS (
        SELECT sl.unified_id, sl.course_id, sl.is_partner, sl.meets_income, sl.cota_eligible,
