...
SELECT sisu_branch.* FROM (
  ...
  FROM public.opportunities o
    ...
    LEFT JOIN LATERAL (
      ...
      JOIN public.campus cap_prev ON cap_prev.id = cp_prev.campus_id
      WHERE opp.opportunity_type = 'sisu' AND cap_prev.institution_id = i.id AND lower(public.f_unaccent(cap_prev.name)) = lower(public.f_unaccent(cp.name)) AND lower(public.f_unaccent(cp_prev.course_name)) = lower(public.f_unaccent(c.course_name)) AND opp.year = COALESCE(p_prev.cycle_year, p.cycle_year - 1) AND (p_prev.id IS NULL OR opp.semester = p_prev.cycle_semester)
    ) s_prev ON true
    ...
    LEFT JOIN LATERAL (
      ...
      JOIN public.campus cap_prev ON cap_prev.id = cp_prev.campus_id
      WHERE cap_prev.institution_id = i.id AND lower(public.f_unaccent(cap_prev.name)) = lower(public.f_unaccent(cp.name)) AND lower(public.f_unaccent(cp_prev.course_name)) = lower(public.f_unaccent(c.course_name)) AND op.year = COALESCE(p_prev.cycle_year, p.cycle_year - 1) AND (p_prev.id IS NULL OR op.semester = p_prev.cycle_semester) AND op.opportunity_type = 'sisu'
      LIMIT 1
    ) sv_prev ON true
    ...
    LEFT JOIN LATERAL (
      ...
      JOIN public.courses cp_prev ON cp_prev.id = op.course_id
      JOIN public.campus cap_prev ON cap_prev.id = cp_prev.campus_id
      WHERE cap_prev.institution_id = i.id AND lower(public.f_unaccent(cap_prev.name)) = lower(public.f_unaccent(cp.name)) AND lower(public.f_unaccent(cp_prev.course_name)) = lower(public.f_unaccent(c.course_name))
        AND op.opportunity_type = 'sisu'
...
BEGIN
  ...
  IF NOT v_has_more THEN
    BEGIN
      ...
      JOIN public.courses c_curr ON c_curr.id = op_curr.course_id
      JOIN public.campus ca_curr ON ca_curr.id = c_curr.campus_id
      WHERE ca_curr.institution_id = ca_prev.institution_id AND lower(public.f_unaccent(ca_curr.name)) = lower(public.f_unaccent(ca_prev.name)) AND lower(public.f_unaccent(c_curr.course_name)) = lower(public.f_unaccent(c_prev.course_name)) 
