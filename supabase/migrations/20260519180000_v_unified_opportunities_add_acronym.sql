-- Add institution_acronym (from institutionsinfosisu.acronym) to v_unified_opportunities
-- v_unified_opportunities is a MATERIALIZED VIEW; v_unified_institutions is a VIEW that depends on it.
-- Order: drop v_unified_institutions → drop mat view → recreate mat view → recreate view
...
UNION ALL
 SELECT 'partner_'::text || po.id::text AS unified_id,
   ...
   FROM partner_opportunities po
     ...
     LEFT JOIN institutionsinfoemec ie ON ie.institution_id = i.id
     LEFT JOIN institutionsinfosisu sis ON sis.institution_id = i.id
  WHERE po.status::text = 'approved'::text;
