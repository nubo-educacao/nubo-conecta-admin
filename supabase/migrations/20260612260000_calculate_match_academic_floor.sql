-- Migration: calculate_match V4.2 — piso do score acadêmico (cap do decay_above)
--
-- MOTIVAÇÃO: o decay_above (0.3/pt) penalizava sem teto quem ficava muito acima da
-- nota de corte. Resultado: candidato com nota máxima (que seria APROVADO) recebia
-- score acadêmico baixíssimo, e só pontuava 100 onde FALTAVA dado de corte. Os três
-- casos extremos divergiam de forma incoerente:
--   - nota inexistente   → acadêmico 50
--   - nota 0 (sob corte)  → acadêmico ~0
--   - muito acima do corte → acadêmico despencava sem limite
--
-- FIX: introduz um piso único (academic_score_floor, default 50, tunável via match_config).
--   - o decay (acima OU abaixo do corte) nunca derruba o acadêmico abaixo do piso;
--   - nota 0 e nota inexistente passam a valer exatamente o piso.
-- Assim o piso é simultaneamente o cap da penalidade e o valor-base de ausência de nota.

-- 1. Config tunável (idempotente)
INSERT INTO public.match_config (weight_key, weight_value, description, category, is_active)
VALUES (
    'academic_score_floor', 50.0,
    'Piso do score acadêmico: cap da penalidade do decay (acima/abaixo do corte) e valor de nota 0 ou inexistente',
    'score_decay', true
)
...
DECLARE
    ...
    -- V4.2: piso do score acadêmico (cap do decay + valor de nota 0/inexistente)
...
BEGIN
    ...
    -- V4.2: resolve o piso acadêmico (cap do decay + valor de nota 0/inexistente)
        ...
        SELECT sp.*,
            CASE
                WHEN sp.weighted_enem_score IS NOT NULL AND sp.cutoff_score IS NOT NULL AND sp.cutoff_score > 0 THEN
                    -- V4.2: piso ancora o decay (acima/abaixo). Acima do corte nunca cai abaixo do piso.
                ...
                WHEN sp.weighted_enem_score IS NOT NULL THEN
                    -- Sem corte: nota baixa também ancora no piso (nota 0 → piso)
                    GREATEST(v_academic_floor, LEAST(100.0, (sp.weighted_enem_score/700.0)*100.0
                        * CASE WHEN v_is_treineiro_score THEN 0.85 ELSE 1.0 END))
                -- Nota inexistente → piso
