-- 20260703170000_prouni_cutoff_agnostic_parse.sql
-- ProUni NU_NOTA_CORTE was parsed by blindly stripping '.', assuming it was always a
-- thousands separator: "450.00" -> "45000" (100x inflated). Real MEC values use '.'
-- as the DECIMAL separator here.
--
-- Fix:
--   1. f_parse_ptbr_numeric(text): format-agnostic parser. Whichever of '.'/',' is
--      closest to the end is treated as the decimal separator; the other (and any
--      repeated separator) is treated as thousands grouping.
--   2. etl_import_prouni: use the helper for cutoff_score (one-line change).
--   3. Backfill: recompute cutoff_score for every existing ProUni opportunity from
--      raw_data->>'NU_NOTA_CORTE' (both the 2025.2 import and the 2026.2 clone, which
--      copied raw_data), then refresh the opportunities matview.

-- ====================================================================================
-- 1. Format-agnostic numeric parser
-- ====================================================================================
CREATE OR REPLACE FUNCTION public.f_parse_ptbr_numeric(p_val text)
RETURNS numeric
LANGUAGE plpgsql
IMMUTABLE
PARALLEL SAFE
...
GRANT EXECUTE ON FUNCTION public.f_parse_ptbr_numeric(text) TO service_role, authenticated, anon;

-- ====================================================================================
-- 2. etl_import_prouni — use the agnostic parser for cutoff_score
--    (identical to 20260703150000 except the cutoff_score expression)
-- ====================================================================================
  ...
  BEGIN
    ...
    mapped_raw AS (
      SELECT
        ...
        public.f_parse_ptbr_numeric(r."NU_NOTA_CORTE") AS cutoff_score,
...
-- ====================================================================================
-- 3. Backfill existing ProUni opportunities from the original raw string, then refresh
--    the opportunities matview (plain REFRESH — runs inside the migration transaction).
-- ====================================================================================
UPDATE public.opportunities
SET cutoff_score = public.f_parse_ptbr_numeric(raw_data->>'NU_NOTA_CORTE'),
    updated_at = now()
WHERE opportunity_type = 'prouni'
  AND raw_data ? 'NU_NOTA_CORTE'
  AND cutoff_score IS DISTINCT FROM public.f_parse_ptbr_numeric(raw_data->>'NU_NOTA_CORTE');
