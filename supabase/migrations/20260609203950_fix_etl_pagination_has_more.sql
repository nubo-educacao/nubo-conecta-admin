-- 20260609203950_fix_etl_pagination_has_more.sql
-- Fixes pagination and loop continuation logic in all 6 ETL functions to count actual raw rows fetched instead of successfully mapped records.
...
BEGIN
  ...
  BEGIN
    -- 1. Insert institutions using CO_IES
    INSERT INTO public.institutions (external_code, name)
    SELECT DISTINCT r."CO_IES"::text, r."NO_IES" 
    ...
    SELECT DISTINCT ca.id, r."CO_IES_CURSO"::text, r."NO_CURSO", r."DS_GRAU"
    FROM (SELECT * FROM public.rawsisu ORDER BY "CO_IES", "NO_CAMPUS", "CO_IES_CURSO", "DS_TURNO", "DS_MOD_CONCORRENCIA" LIMIT p_limit OFFSET p_offset) r
    JOIN public.institutions i ON i.external_code = r."CO_IES"::text
    JOIN public.campus ca ON ca.institution_id = i.id AND ca.name = r."NO_CAMPUS"
    WHERE r."CO_IES_CURSO" IS NOT NULL
    ...
    mapped_raw AS (
      ...
      JOIN public.institutions i ON i.external_code = r."CO_IES"::text
      JOIN public.campus ca ON ca.institution_id = i.id AND ca.name = r."NO_CAMPUS"
      JOIN public.courses c ON c.campus_id = ca.id AND c.course_code = r."CO_IES_CURSO"::text
    ),
    mapped AS (
  ...
  IF NOT v_has_more THEN
    ...
    SELECT COUNT(DISTINCT ("CO_IES"::text || '|' || "NO_CAMPUS")) INTO v_campus_count FROM public.rawsisu WHERE "CO_IES" IS NOT NULL AND "NO_CAMPUS" IS NOT NULL;
    SELECT COUNT(DISTINCT "CO_IES_CURSO") INTO v_course_count FROM public.rawsisu WHERE "CO_IES_CURSO" IS NOT NULL;
...
BEGIN
  ...
  IF p_limit IS NOT NULL THEN
    SELECT COUNT(*) INTO v_batch_rows FROM (
      SELECT 1 FROM public.rawprouni LIMIT p_limit OFFSET p_offset
    ) sub;
    v_has_more := (v_batch_rows = p_limit);
  ELSE
    v_has_more := FALSE;
...
BEGIN
  ...
  IF p_limit IS NOT NULL THEN
    SELECT COUNT(*) INTO v_batch_rows FROM (
      SELECT 1 FROM public.rawprounivacancies LIMIT p_limit OFFSET p_offset
    ) sub;
    v_has_more := (v_batch_rows = p_limit);
  ELSE
    v_has_more := FALSE;
...
BEGIN
  ...
  IF p_limit IS NOT NULL THEN
    SELECT COUNT(*) INTO v_batch_rows FROM (
      SELECT 1 FROM public.rawprouniocuppied LIMIT p_limit OFFSET p_offset
    ) sub;
    v_has_more := (v_batch_rows = p_limit);
  ELSE
    v_has_more := FALSE;
