-- =============================================================================
-- Migration: calculate_match V3 — Funnel Cap (Sprint 10 hotfix)
--
-- Problem: with broad filters (e.g. program=prouni + 3 shifts, no state/course)
--   the funnel still produces ~12k rows, which exceeds Supabase's ~30s statement
--   timeout when all 6 CTE scoring stages run.
--
-- Fix: wrap each MEC funnel path in a subquery with ORDER BY + LIMIT 3000.
--   Ordering by cutoff_score DESC ensures the most competitive (relevant)
--   opportunities are scored first — preserving ranking quality.
--   Partners path is unaffected (always 4 rows, no cap needed).
-- =============================================================================
...
BEGIN
    ...
    -- ── MEC funnel — each path capped at 3000, ordered by cutoff_score DESC ──
    mec_funnel AS (
    ...
    -- ── Partners: always ALL approved, no cap ────────────────────────────────
    ...
    SELECT 1 FROM mec_insert, partner_insert LIMIT 0;

END;
$$;

COMMENT ON FUNCTION public.calculate_match IS
    'V3 Funnel+Cap: each MEC path capped at 3000 rows (ORDER BY cutoff_score DESC) '
    'before scoring to prevent statement timeout. Partners always included uncapped. '
    'Persists TOP 100 MEC + ALL partners into user_opportunity_matches.';
