...
SELECT sisu_branch.* FROM (
  ...
  FROM public.opportunities o
    ...
    LEFT JOIN LATERAL (
      SELECT min(opp.cutoff_score) AS min_cutoff, max(opp.cutoff_score) AS max_cutoff
      FROM public.opportunities opp
      WHERE opp.opportunity_type = 'sisu' AND opp.course_id = o.course_id AND opp.year = COALESCE(p_prev.cycle_year, p.cycle_year - 1) AND (p_prev.id IS NULL OR opp.semester = p_prev.cycle_semester)
    ) s_prev ON true
    ...
    LEFT JOIN LATERAL (
      ...
      FROM public.opportunities_sisu_vacancies sv
      JOIN public.opportunities op ON op.id = sv.opportunity_id
      WHERE op.course_id = o.course_id AND op.year = COALESCE(p_prev.cycle_year, p.cycle_year - 1) AND (p_prev.id IS NULL OR op.semester = p_prev.cycle_semester) AND op.opportunity_type = 'sisu'
      LIMIT 1
    ) sv_prev ON true
...
BEGIN
  ...
  IF NOT v_has_more THEN
    BEGIN
      ...
      WHERE op_curr.opportunity_type = 'sisu' AND op_curr.year = v_year AND op_curr.semester = v_semester AND op_prev.opportunity_type = 'sisu' AND op_prev.year = v_prev_year AND op_prev.semester = v_prev_semester AND op_curr.course_id = op_prev.course_id AND op_curr.shift = op_prev.shift AND op_curr.concurrency_type = op_prev.concurrency_type AND op_prev.cutoff_score IS NOT NULL AND op_curr.cutoff_score IS NULL;
      ...
      JOIN public.opportunities o_prev ON o_prev.course_id = o_curr.course_id AND o_prev.shift = o_curr.shift AND o_prev.concurrency_type = o_curr.concurrency_type AND o_prev.year = v_prev_year AND o_prev.semester = v_prev_semester AND o_prev.opportunity_type = 'sisu'
