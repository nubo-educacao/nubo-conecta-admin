-- Sprint 13.0 Hotfix — Consistência de Dados
-- 1. Corrige dados legados 'bolsa' para 'programa de bolsa' em partner_opportunities
-- 2. Atualiza motor de match V3 para referenciar a tabela renomeada opportunities_sisu_vacancies
-- 3. Atualiza motor de match V3 para suportar os novos status de partner_opportunities ('opened', 'incoming', 'closed')

UPDATE partner_opportunities
SET opportunity_type = 'programa de bolsa'
WHERE opportunity_type = 'bolsa';

REFRESH MATERIALIZED VIEW v_unified_opportunities;

-- Recreate calculate_match RPC with updated table name and status filter
...
COMMENT ON FUNCTION public.calculate_match IS
    'V3 Funnel+Cap: Fix para utilizar opportunities_sisu_vacancies no lugar da tabela legada. '
    'E adiciona suporte aos novos status de partner_opportunities (incoming, opened, closed).';
